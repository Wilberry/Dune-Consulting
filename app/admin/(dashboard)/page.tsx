    ...
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="text-navy mt-2 text-3xl font-extrabold sm:text-4xl">Dashboard</h1>
      ...
      {params.access === "restricted" && (
        <div className="border-amber/50 bg-amber/10 text-navy mt-6 rounded-lg border p-4 text-sm" role="status">
      ...
      {loadError ? (
        <div className="border-line mt-8 rounded-xl border bg-white p-6">
          <h2 className="text-navy text-lg font-bold">Dashboard data unavailable</h2>
          <p className="text-muted mt-2 text-sm leading-6">
            The admin shell is working, but Supabase data could not be loaded. Check the project environment variables and migration status.
          </p>
        </div>
      ) : (
        <>
            ...
            {dashboard?.stats.map((stat) => (
              ...
              >
                ...
                <div className="mt-4 flex items-end justify-between gap-4">
                  ...
                  {!stat.restricted && (
                    <ArrowRight className="text-amber-hover transition-transform group-hover:translate-x-1" size={19} aria-hidden="true" />
                  )}
                </div>
              </Link>
            ))}
            ...
            {dashboard?.activities.length ? (
              <div className="divide-line divide-y">
                {dashboard.activities.map((activity) => (
                  ...
                  >
                    <div>
                      <p className="text-navy text-sm font-bold">{activity.title}</p>
                      <p className="text-muted mt-1 text-xs">{activity.type} · {formatDate(activity.createdAt)}</p>
                    </div>
              ...
              <div className="px-5 py-10 text-center sm:px-6">
                ...
                <p className="text-muted mt-2 text-sm">New articles and submissions will appear here once records exist.</p>
              </div>
            )}
          </section>
        </>
