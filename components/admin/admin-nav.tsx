...
function NavigationItems({ close }: { close?: () => void }) {
  ...
  return (
    <nav aria-label="Admin navigation" className="space-y-1">
      {items.map(({ href, label, icon: Icon }) => {
        ...
        return (
          <Link key={href} href={href} onClick={close} className={cn("flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-semibold transition", active ? "bg-amber text-deep-navy" : "text-white/80 hover:bg-white/10 hover:text-white")}>
...
export function AdminSidebar() {
  return <aside className="bg-deep-navy hidden min-h-screen w-72 shrink-0 p-5 lg:block"><NavigationItems /></aside>;
}
      ...
      {open && (
        <div className="fixed inset-0 z-[90] lg:hidden">
          ...
          <aside className="bg-deep-navy relative z-10 h-full w-[min(86vw,320px)] p-5 shadow-2xl">
            ...
            <NavigationItems close={() => setOpen(false)} />
          </aside>
        </div>
      )}
    </>
